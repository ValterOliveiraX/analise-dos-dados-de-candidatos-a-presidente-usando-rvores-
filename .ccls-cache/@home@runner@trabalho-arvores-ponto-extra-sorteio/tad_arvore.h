...
void adicionar_elemento_arvore(tree *t, elemento e,
                               int(comparador)(elemento a, elemento b)) {
  if (vazia(*t)) {
    criaRaiz(t, e);
    return;
  }
  ...
  while (1) {
    if (comparador(parente->info, e) > 1) {
      if (parente->esq == NULL) {
        criarNodoEsq(parente, e);
        break;
      }
